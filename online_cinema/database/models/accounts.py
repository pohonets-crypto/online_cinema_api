...
class UserGroupModel(Base):
    ...
    name: Mapped[UserGroupEnum] = mapped_column(
        Enum(UserGroupEnum, native_enum=False, length=50), nullable=False, unique=True
    )
...
class UserModel(Base):
    ...
    favorite_movies = relationship(
        "MovieModel", secondary="favorite_movies", back_populates="favorited_by"
    )
    ...
    cart: Mapped[Optional["CartModel"]] = relationship(
        "CartModel", back_populates="user", cascade="all, delete-orphan", uselist=False
    )
...
class UserProfileModel(Base):
    ...
    gender: Mapped[Optional[GenderEnum]] = mapped_column(
        Enum(GenderEnum, native_enum=False, length=10)
    )
